...
import { downloadFileFn, downloadFileStreamFn } from '../functions'
...
export function DownloadFileModal({
  ...
  const downloadMutation = useMutation({
    mutationFn: async () => {
      ...
      const downloadInfo = await downloadFileFn({ data: file.id })
      ...
      const encryptedBase64 = await downloadFileStreamFn({ data: file.id })
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Tài liệu được mã hóa đầu cuối. Vui lòng xác thực để giải mã và tải xuống.
          </DialogDescription>
        </DialogHeader>
