...
import { uploadFileFn } from '../functions'
import {
  generateAesKey,
  encryptFileData,
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <div className="flex flex-col gap-6 py-2">
          ...
          <div className="space-y-2">
            ...
            {selectedFiles.length === 0 ? (
              ...
              >
                ...
                <div className="font-medium text-foreground">Nhấn để chọn hoặc kéo thả tệp vào đây</div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium text-muted-foreground">Đã chọn {selectedFiles.length} tệp</span>
                  <Button variant="ghost" size="sm" onClick={() => document.getElementById('file-upload')?.click()} className="h-8 text-xs">
                    + Thêm tệp
                  </Button>
                </div>
                <div className="max-h-[180px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                  {selectedFiles.map((file, index) => (
                    <div key={index} className="flex items-center gap-3 bg-muted/40 p-2 rounded-md border border-border group hover:border-primary/30 transition-colors">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {/* Passcode */}
            <div className="space-y-2">
              <Label htmlFor="passcode" className="flex items-center justify-between">
        ...
        <DialogFooter className="gap-2 sm:gap-0 space-x-2">
          ...
          <Button
            ...
            disabled={
              selectedFiles.length === 0 || !passcode || uploadMutation.isPending
            }
