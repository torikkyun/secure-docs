...
import { shareFileFn } from '../functions'
import { getUsersFn } from '../../functions'
import {
  decryptPrivateKey,
  deriveSharedSecret,
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Chia sẻ <span className="font-medium text-foreground break-all">{file?.filename}</span> với người dùng khác.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 p-2 cursor-pointer hover:bg-muted transition-colors ${
                        (isOwner || isSelected) ? 'opacity-50 cursor-not-allowed bg-muted/50' : ''
                      }`}
                      onClick={() => !isOwner && !isSelected && handleSelectUser(user)}
                    >
                      ...
                      {isOwner && <span className="text-xs text-muted-foreground">Chủ sở hữu</span>}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1">
                {selectedUsers.map((user) => (
                  <Badge key={user.id} variant="secondary" className="pl-1 pr-2 py-1 h-8 flex items-center gap-2">
          ...
          <div className="space-y-2 pt-2 border-t mt-4">
            <Label htmlFor="passcode" className="flex justify-between items-center">
